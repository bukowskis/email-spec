...
module EmailSpec
  ...
  module Helpers
    ...
    def click_email_link_matching(regex, email = current_email)
      ...
      raise "No link found matching #{regex.inspect} in #{email.default_part_body_text}" unless url
    ...
    def find_email(address, opts={})
      ...
      if opts[:with_subject]
        ...
        mailbox_for(address).find { |m| m.default_part_body_text =~ Regexp.new(opts[:with_text]) }
      else
    ...
    def links_in_email(email)
      URI.extract(email.default_part_body_text, ['http', 'https'])
    end
    ...
    def parse_email_for_anchor_text_link(email, link_text)
      if textify_images(email.default_part_body_text) =~ %r{<a[^>]*href=['"]?([^'"]*)['"]?[^>]*?>[^<]*?#{link_text}[^<]*?</a>}
